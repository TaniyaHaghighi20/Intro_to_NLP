...
def find_subset_indices(nums, target, tolerance=0):
    """
    Find the first subset of indices in nums whose corresponding values sum to target (+/- tolerance).
    """
    for r in range(1, len(nums)+1):
        for combo in itertools.combinations(enumerate(nums), r):
...
def get_kermany_imgs(data_dir: str, **kwargs):
    ...
    for img_file in img_filename_list:  # iterate over each class
        ...
        for i, percentage in enumerate(copy_split):
            ...
            #     break
            subset = find_subset_indices(num_visits, selected_num, tolerance=10)  # tolerance can be 0 or higher
...
def get_OLIVE_imgs(root, classes):
    img_paths = []
    excel_data = pd.read_excel(os.path.join(root,"OLIVES_Dataset_Labels/ml_centric_labels","Clinical_Data_Images.xlsx"))['File_Path']
    img_paths = [(os.path.join(root, "OLIVES"+ row),(get_class(classes[0], classes),classes[0])) for row in excel_data]
    return img_paths
     
def get_Fairhub_imgs(root, classes):
    img_paths = []
    for _, _, files in os.walk(root):
        # Filter for .png files
        img_paths += [(f,(get_class(classes[0], classes),classes[0])) for f in files if f.lower().endswith('.png')]
    return img_paths
