...
class Classifier(nn.Module):
    def __init__(self, encoder, num_classes):
        ...
        hidden_dim = 1152  # For DiT-XL/2, adjust accordingly if using another model
        self.cls_head = MLP(hidden_dim  , num_classes)
        self.criterion = FocalLoss()
        self.criterion = nn.CrossEntropyLoss()

    def forward(self, x, t, y):
        features = self.encoder.forward(x, t, y, return_features=True)
        features = features.mean(dim=1)  # Global average pooling over patches -> (N, D)
